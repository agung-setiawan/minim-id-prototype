...
export default class ShortenChannel extends Component {
  constructor(props) {
    super(props);
    this.state = {
      time: 10,
  ...
  componentDidMount = async e => {
    ...
    const body = JSON.stringify({ hostName, channel, slug });
    ...
    const res = await axios.post(
      "/api/shortener/getShortenChannelLink",
      body,
      config
    );
    ...
    if (res.data.theData.length > 0) {
      if (res.data.theData[0].url.length > 0) {
        this.setState({ isVerified: true, theURL: res.data.theData[0].url });
        this.start();
      }
    }
  };
              ...
              <div className="row h-100">
                <div className="col-12 my-auto">
                  <div className="masthead-content text-white py-5 py-md-0">
                    <h1 className="mb-3">Advertiser Here!</h1>
                    <p className="mb-5">
